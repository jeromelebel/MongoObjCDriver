...
static void testObjects(NSString *jsonToParse, NSString *jsonExpected, id shouldEqual)
{
    ...
    [MODJsonToObjectAssembler bsonFromJson:&bsonResult json:jsonToParse error:&error];
    ...
    objects = [MODJsonToObjectAssembler objectsFromJson:jsonToParse error:&error];
    if (error) {
        NSLog(@"***** parsing errors for:");
        NSLog(@"%@", jsonToParse);
        NSLog(@"%@", error);
...
static void testJson()
{
    ...
    testObjects(@"{\"number\":3.8365551715863E-13}", @"{\"number\":3.8365551715863000336e-13}", [MODSortedMutableDictionary sortedDictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:3.8365551715863e-13], @"number", nil]);
    ...
    testObjects(@"{\"number\":1.2345678910000000}", @"{\"number\":1.2345678909999999728}", [MODSortedMutableDictionary sortedDictionaryWithObjectsAndKeys:[NSNumber numberWithDouble:1.234567891], @"number", nil]);
    ...
    // https://github.com/fotonauts/MongoHub-Mac/issues/28
    {
        ...
        [MODJsonToObjectAssembler bsonFromJson:&bsonObject json:@"{ \"array\": [ 1, {\"x\": 1}, [ 1 ]] }" error:&error];
