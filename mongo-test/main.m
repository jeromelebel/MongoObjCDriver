...
static void testObjects(NSString *jsonToParse, NSString *jsonExpected, id shouldEqual)
{
    ...
    [MODJsonToBsonParser bsonFromJson:&bsonResult json:jsonToParse error:&error];
    ...
    objects = [MODJsonToObjectParser objectsFromJson:jsonToParse error:&error];
    if (error) {
        NSLog(@"***** parsing errors for:");
        NSLog(@"%@", jsonToParse);
        NSLog(@"%@", error);
...
static void testJson()
{
    ...
    testObjects(@"{\"mydate\":{\"$date\":1320066612000}}", nil, [MODSortedMutableDictionary sortedDictionaryWithObjectsAndKeys:[[[NSDate alloc] initWithTimeIntervalSince1970:1320066612] autorelease], @"mydate", nil]);
    ...
    testObjects(@"{\"undefined value\":{\"$undefined\":true}}", nil, [MODSortedMutableDictionary sortedDictionaryWithObjectsAndKeys:[[[MODUndefined alloc] init] autorelease], @"undefined value", nil]);
